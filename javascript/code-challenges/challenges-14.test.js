...
const screenForNames = (arr) => {
  let nameCriteria = /^(?:Mrs|Mr|Dr|Ms)+.(?:\s)([A-Z][a-z]+)/g;
  return arr.filter(currentName => nameCriteria.test(currentName));
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  let secureCriteria = /^(?:https:\/\/)/;
  return secureCriteria.test(url);
};
