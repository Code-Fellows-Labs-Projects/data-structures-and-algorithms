...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(number => {
    newNumberArray.push(Math.pow(number, 3));
  });
...
const evenOdd = (arr) => {
  let evenOddArray = arr.map(value => {
    if (value % 2) {
      return 'odd';
    } else if (value % 2 === 0) {
      return 'even';
...
const extractAbilities = (arr) => {
  let abilityName = arr.map(currentValue => {
    return currentValue.ability.name;
  });
  return abilityName;
};
...
const extractStats = (arr) => {
  let statsArray = arr.map(currentValue => (
    {
      name: currentValue.stat.name,
      total: currentValue.effort + currentValue.baseStat,
    }
  ));
  return statsArray;
};
