...
const sortByChildren = (charArray) => {
  let fewestToMost = charArray.sort((a,b) => {
...
const matchMonth = (input) => {
  const octoberRegex = /^([Oo]ct(?:ober)?)$/;
  return octoberRegex.test(input);
};
...
const noPunctuation = str => {
  const noPunctionRegex = /\b\w+\s/g;
  return str.match(noPunctionRegex);
};
