...
const containsAnd = (arr) => {
  let andArray = arr.filter(letters => letters.includes('and'));
  return andArray;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let greaterThanBaseStatsArr = arr.filter( value => {
    return value.baseStat > minBaseStat;
  });
  return greaterThanBaseStatsArr;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  let nameArray = greaterThanBaseStatsArr.map( value => {
...
const getCharactersWithoutChildren = (arr) => {
  let noKiddos = arr.filter( character => {
...
const evenOddNumericValues = (arr) => {
  let onlyNumbers = arr.filter( value => {
    return typeof value === 'number';
  });
  let evenAndOdds = onlyNumbers.map( value => {
    if (value % 2) {
      return 'odd';
    } else if (!(value % 2)) {
      return 'even';
