...
const AnimalShelter = require('../classes/animalShelter');
...
describe('Testing Animal Shelter Queue', () => {
  ...
  it('Can successfully enqueue cat and dog', () => {
    ...
    expect(shelter.dogs.front.value.name).toStrictEqual('Choco');
    expect(shelter.cats.front.value.name).toStrictEqual('Vladimir');
  });
  ...
  it('Can successfully enqueue multiple cats and dogs', () => {
    ...
    expect(shelter.dogs.front.value.name).toStrictEqual('Choco');
    expect(shelter.dogs.back.value.name).toStrictEqual('Pip');

    shelter.enqueue(cat1);
    shelter.enqueue(cat2);
    shelter.enqueue(cat3);
    expect(shelter.cats.front.value.name).toStrictEqual('Vladimir');
    expect(shelter.cats.back.value.name).toStrictEqual('Mochi');
  });
